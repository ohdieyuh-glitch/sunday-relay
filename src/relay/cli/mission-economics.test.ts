...
describe('mission economics command parsing', () => {
  ...
  it('rejects a missing or unknown action rather than guessing', () => {
    expect(parseCli(['mission']).error).toMatch(/economics, budget, receipts, worktree, coding-agent, or prompt-architect/u);
    expect(parseCli(['mission', 'spend']).error).toMatch(/economics, budget, receipts, worktree, coding-agent, or prompt-architect/u);
  });
});
